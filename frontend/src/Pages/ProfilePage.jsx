  ...
  useEffect(() => {
    (() => {
      ...
      if (CheckoutStateData.length)
        setOrder(CheckoutStateData.filter((x) => x.user === localStorage.getItem("userid")))
    })()
    ...
    <>
      ...
      <div className="container-fluid my-3">
                ...
                <tbody>
                  {
                    wishlist?.map((item) => {
                      return <tr key={item.id}>
                        ...
                        <td><Link to={`/product/${item.product}`}><i className='fa fa-shopping-cart'></i></Link></td>
        ...
        <h5 className='bg-primary p-2 text-center text-light'>Orders History Section</h5>
        {
          order.length ?
            order.map((item) => {
              return <div key={item.id} className="row border-bottom border-5 border-primary">
