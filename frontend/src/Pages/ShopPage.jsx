...
export default function ShopPage() {
  ...
  function postSearch(e) {
    e.preventDefault()
    setProduct(ProductStateData.filter((x) => x.name?.toLocaleLowerCase().includes(search) ||
  ...
  function applyFilter(mc, sc, br, min = -1, max = -1) {
    ...
    if (mc === "All" && sc === "All" && br === "All")
      ...
      data = ProductStateData.filter((x) => x.active && x.subcategory === sc && x.brand === br)
    else
      data = ProductStateData.filter((x) => x.active && x.maincategory === mc && x.subcategory === sc && x.brand === br)
      ...
      <div className="container-fluid my-3">
        <div className="row">
          <div className="col-md-2">
            ...
            <div className="mb-3">
              <h5 className='bg-primary text-center text-light p-2'>Price Range</h5>
              <form onSubmit={postPriceFilter}>
                <div className="row">
                  <div className="col-md-6 mb-3">
                    <label>Min Amount</label>
                    <input type='number' name='min' value={min} onChange={(e) => setMin(e.target.value)} className='form-control border-3 border-primary' placeholder='Min Amount.' />
                  </div>

                  <div className="col-md-6 mb-3">
                    <label>Max Amount</label>
                    <input type='number' name='max' value={max} onChange={(e) => setMax(e.target.value)} className='form-control border-3 border-primary' placeholder='Max Amount.' />
                  </div>
                </div>
                <div className="mb-3">
          ...
          <div className="col-md-10">
              ...
              <div className="col-md-9">
                <form onSubmit={postSearch}>
                  <div className="btn-group w-100">
                    <input type="search" name="search" value={search} onChange={(e) => setSearch(e.target.value.toLocaleLowerCase())} placeholder='Search Products by Name, Maincategory, Subcategory, Brand, Description...' className='form-control border-3 border-primary' />
            ...
            <Products data={product} title="shop" />
          </div>
        </div>
      </div>
      <Testinomial />
