...
export default function AdminUpdateSubcategory() {
    ...
    function postData(e) {
        ...
        if (error)
            ...
            let item = SubcategoryStateData.find((x) => x.name.toLocaleLowerCase() === data.name.toLocaleLowerCase() && x.id != id)    // current product nhi hona chahiye koi aur hona chahiye  
            if (item) {
                setShow(true)
                setErrorMessage((old) => {
                    return {
                        ...
                        <form onSubmit={postData}>
                            ...
                            <div className="row">
                                <div className="col-md-6 mb-3">
                                    <label>Pic*</label>
                                    <input type="file" name='pic' onChange={getInputData} placeholder='Subcategory Name' className={`form-control border-3 ${show && errorMessage.pic ? 'border-danger' : 'border-primary'}`} />
